...
{
    // Handle Ctrl+letter (key = ASCII control code 1-26) → <C-x>
    if ((modifierKeys & ModifierKey::Ctrl) && key >= 1 && key <= 26)
    {
        char c = 'a' + (key - 1); // map 1→'a', 2→'b', ... 26→'z'
...
bool ZepMode::GetCommand(CommandContext& context)
{
    ...
    else if (mappedCommand == id_StandardCopy)
    {
        ...
        // If in Visual mode, use the visual selection (m_visualBegin/End)
        // Otherwise, fall back to the buffer's selection (e.g., from mouse drag)
        GlyphRange range;
        bool usingVisual = false;
        if (context.currentMode == EditorMode::Visual)
        {
            range = GetInclusiveVisualRange();
            usingVisual = true;
        }
        else
        {
            ...
            if (buf.HasSelection())
            {
                range = buf.GetInclusiveSelection();
            }
            else
            {
                // No explicit selection; nothing to copy
                return true;
            }
        }
        ...
        // If copying from visual selection, stay in Visual mode to keep highlighting
        if (usingVisual)
        {
            context.commandResult.modeSwitch = EditorMode::Visual;
        }
    ...
    else
    {
        ...
        {
            // Fallback to visual range (might be empty)
            range = GetInclusiveVisualRange();
            if (!range.Valid())
                return true;
        }
    ...
    // Copy in standard mode stays in visual mode (if we used visual)
    context.commandResult.modeSwitch = EditorMode::Visual;
    context.registers.push('0');
    context.registers.push('*');
    context.registers.push('+');
